...
class Application(tk.Tk):
    def __init__(self): 
        ...
        self.lyrics_data = []
        self.selected_mic_index = self._get_best_microphone()
    ...
    def show_Menu(self):
        ...
        button_play = tk.Button(self.frame, text="Play !", command=self.show_play)
    ...
    def show_play(self):
        ...
        files = [file for file in os.listdir(self.audio_dir) if os.path.isfile(os.path.join(self.audio_dir, file))]
        for i, file in enumerate(files, start=1):  # Start from row 1 as the label is in row 0
    ...
    def item_clicked(self, selected_item):
        response = messagebox.askyesno(f"You choosed the song {selected_item}", "Would you like to listen to the song first ?")
        self.scene_history.append(self.show_play)
        if response:
    ...
    def setup_karaoke_ui(self, selected_item):
        ...
        self.lyrics_label = tk.Label(self.frame, font=("Arial", 18), width=80, height=20)
        self.lyrics_label.grid(row=3, column=5, pady=40)

        self.progress_bar = ttk.Progressbar(self.frame, length=500, mode='determinate')
        self.progress_bar.grid(row=9, column=5, pady=20)

        self.countdown_label = tk.Label(self.frame, font=("Arial", 18))
        self.countdown_label.grid(row=11, column=5, pady=20)
